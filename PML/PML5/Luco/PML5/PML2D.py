...
structureylist = np.arange(0, hstructure + eps, dy)
...
# find the minimum node tag with domain==strucrure
regendnode = nodes[nodes['Domain'] == 'Structure']["Tag"].min()
nsy = int(hstructure/dy + eps)
nsx = int(bstructure/dx + eps)
for i in range(nsx):
    for j in range(nsy):
        node1 = i * (nsy + 1) + j + 1 + regendnode -1
        node2 = node1 + nsy + 1
        node3 = node2 + 1 
        node4 = node1 + 1
        element_data = {'node1': node1, 'node2': node2,
                        'node3': node3, 'node4': node4, 'Domain': 'Structure'}
        elements = pd.concat([elements, pd.DataFrame([element_data])], ignore_index=True)
...
# =======================================================================
# create Structure nodes file
# =======================================================================
structurenodes = nodes[nodes['Domain'] == 'Structure']
...
for core in range(regcores):
    ...
    # filter nodes which are in the core
    structurenodes = nodes[nodes['Domain'] == 'Structure']
    nodes_in_core = structurenodes[structurenodes['status'] == 1]
...
# =======================================================================
# create Structure elements file
# =======================================================================
# iterate over regcores to create a dataframe for each core
for core in range(regcores):
    ...
    for _, ele in eles.iterrows():
        file.write('eval "element quad %d %d %d %d %d $Structurematerial"\n' % (ele['tag'], ele['node1'], ele['node2'], ele['node3'], ele['node4']))
    file.close()
...
# =======================================================================
# create soil-structure interface file
# =======================================================================
# find the nodetags
interface = nodes[(nodes['X'] >= -bstructure/2.-eps) & (nodes['X'] <= +bstructure/2.+eps) & np.isclose(nodes['Y'],  0.0, atol=eps)]
soilinterface = interface[interface['Domain'] == 'Soil']
structureinterface = interface[interface['Domain'] == 'Structure']

# check if the size of the soilinterface dataframe is equal to the size of the structureinterface dataframe and raise an error if not
if soilinterface.shape[0] != structureinterface.shape[0]:
    raise ValueError('The number of soil interface nodes is not equal to the number of structure interface nodes')

file = open('interface.tcl', 'w')
for _, (soilrow, structurerow) in enumerate(zip(soilinterface.iterrows(), structureinterface.iterrows())):
    # check the coordinates of the nodes are equal or not and raise an error if not
    # using np.isclose because of the floating point precision
    if not (np.isclose(soilrow[1]['X'], structurerow[1]['X'], atol=eps) and np.isclose(soilrow[1]['Y'], structurerow[1]['Y'], atol=eps)):
        raise ValueError('The coordinates of the soil and structure interface nodes are not equal')
    file.write('equalDOF %d %d 1 2\n' % (soilrow[1]['Tag'], structurerow[1]['Tag']))
file.close()
# =============================================================================
# create loading file
# =============================================================================
# find the nodetags
# loadlist = nodes[np.isclose(nodes['X'], 0.0, atol=eps) & np.isclose(nodes['Y'],  hstructure, atol=eps)]
loadlist = nodes[np.isclose(nodes['X'], 0.0, atol=eps) & np.isclose(nodes['Y'],  0.0, atol=eps)]
