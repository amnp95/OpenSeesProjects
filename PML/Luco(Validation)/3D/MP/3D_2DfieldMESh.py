...
bstructure   = float(sys.argv[10])
hstructure   = float(sys.argv[11])
wstructure   = float(sys.argv[12])
...
print('regular cores = %d, pml cores = %d, lx = %f, ly = %f, lz = %f, dx = %f, dy = %f, dz = %f, pmlthickness = %f, bstructure = %f, hstructure = %f, wstructure = %f' % (regcores, pmlcores, lx, ly, lz, dx, dy, dz, pmlthickness, bstructure, hstructure, wstructure))
...
structurexlist = np.arange(-bstructure/2, bstructure/2 + eps, dx)
structureylist = np.arange(-wstructure/2, wstructure/2 + eps, dy)
structurezlist = np.arange(0, hstructure + eps, dz)
...
# Adding structure nodes to the nodes dataframe
for x in structurexlist:
    for y in structureylist:
        for z in structurezlist:
                nodes_data = {'x':x ,
                              'y': y,
                              'z': z,
                              'Domain': 'structure'}
...
# find the first node of the structure
firstnode = nodes[nodes['Domain'] == 'structure']["tag"].values[0]
nsy = int(wstructure/dy + eps)
nsz = int(hstructure/dz + eps)
nsx = int(bstructure/dx + eps)
for x in range(int(bstructure/dx + eps)):
    for y in range(int(wstructure/dy + eps)):
        for z in range(int(hstructure/dz + eps)):
            node1 = int(x * (nsy + 1) * (nsz + 1) + y * (nsz + 1) + z + firstnode)
            node2 = int((x + 1) * (nsy + 1) * (nsz + 1) + y * (nsz + 1) + z + firstnode)
            node3 = int((x + 1) * (nsy + 1) * (nsz + 1) + (y + 1) * (nsz + 1) + z + firstnode)
            node4 = int(x * (nsy + 1) * (nsz + 1) + (y + 1) * (nsz + 1) + z + firstnode)
            node5 = node1 + 1
            node6 = node2 + 1
            node7 = node3 + 1
            node8 = node4 + 1
...
for core in range(regcores):
    # filter reg elements
    eles = elements[(elements['core'] == core) & (elements['Domain'] == 'reg')]
    ...
    nodes['status'] = 0
# =============================================================================
# create structure node file
# =============================================================================
# iterate over regcores to create a dataframe for each core
for core in range(regcores):
    # filter reg elements
    eles = elements[(elements['core'] == core) & (elements['Domain'] == 'structure')]
    # iterate over elements to node1 to node8 and set their status to 1
    for _, ele in eles.iterrows():
    ...
    # write the nodes in a file
    file = open('structurenodes'+str(core)+'.tcl', 'w')
    for _, node in nodes_in_core.iterrows():
        file.write('node %d %f %f %f\n' % (node['tag'], node['x'], node['y'], node['z']))
    file.close()

    # adding fixity to the nodes
    file = open('structurefixity'+str(core)+'.tcl', 'w')
    for _, node in nodes_in_core.iterrows():
        file.write('fix %d 0 1 0\n' % (node['tag']))
    file.close()
...
for core in range(pmlcores):
    # filter pml elements
    eles = elements[(elements['core'] == core + regcores) & (elements['Domain'] == 'pml') ]
...
for core in range(regcores):
    ...
    # write a file for partioning
    file = open('structureelements'+str(core)+'.tcl', 'w')

    # iterate over the rows of reg elements dataframe to write in file
    for _, row in eles.iterrows():
        file.write('element stdBrick %d %d %d %d %d %d %d %d %d $StructureMaterialTag\n' % (row['tag'], row['node1'], row['node2'], row['node3'], row['node4'], row['node5'], row['node6'], row['node7'], row['node8']))
    file.close()

# %%
# =============================================================================
# create file to tie soil and structure nodes 
# =============================================================================
nodes['boundary'] = 0

tolerance = 1e-6  # Adjust the tolerance based on your needs
# set boundary nodes to 1 if x between -bstructure/2. - tolerance and bstructure/2. + tolerance and z is equal to 0
nodes.loc[(nodes['x'] >= -bstructure/2. - tolerance) & (nodes['x'] <= bstructure/2. + tolerance) & (np.isclose(nodes['z'], 0, atol=tolerance)), 'boundary'] = 1
 
# seperate soil nodes which has boundary 1 and domain reg
soil_nodes = nodes[(nodes['boundary'] == 1) & (nodes['Domain'] == 'reg')]

# seperate structure nodes which has boundary 1 and domain structure
structure_nodes = nodes[(nodes['boundary'] == 1) & (nodes['Domain'] == 'structure')]

# check that the number of soil nodes and structure nodes are equal
if len(soil_nodes) != len(structure_nodes):
    print('number of reg nodes and structure nodes are not equal')
    print('number of reg nodes: ', len(soil_nodes)) 
    print('number of structure nodes: ', len(structure_nodes))
    sys.exit()

# write the file
file = open('soilstructure.tcl', 'w')
for i in range(len(structure_nodes)):
    file.write('equalDOF %d %d 1 3\n' % (structure_nodes.iloc[i]['tag'], soil_nodes.iloc[i]['tag']))
file.close()

# delete soil_nodes, structure_nodes
del soil_nodes, structure_nodes
...
# set the core of the elements with domain structure to 1
# elements.loc[elements['Domain'] == 'structure', 'core'] = regcores + pmlcores
# %%
...
# =============================================================================
# find the nodetags
nodetag1 = nodes[np.isclose(nodes['x'], 0., atol=tolerance) & np.isclose(nodes['y'],  ly/2., atol=tolerance) & np.isclose(nodes['z'], 0, atol=tolerance)]
nodetag2 = nodes[np.isclose(nodes['x'], 0., atol=tolerance) & np.isclose(nodes['y'], -ly/2., atol=tolerance) & np.isclose(nodes['z'], 0, atol=tolerance)]

# filter the nodes in nodetag1 with domain structure
nodetag1 = nodetag1[nodetag1['Domain'] == 'structure']
nodetag2 = nodetag2[nodetag2['Domain'] == 'structure']

# write the node tag in file
file = open('load.tcl', 'w')
file.write('if {$pid == %d}  {load %d 0 0 1.0}\n' % (nodetag1['core'].values[0], nodetag1['tag'].values[0]))
...
nodetag1 = nodetag1['tag'].values[0]
nodetag2 = nodes[np.isclose(nodes['x'], lx/2., atol=tolerance) & np.isclose(nodes['y'],  ly/2, atol=tolerance) & np.isclose(nodes['z'], 0, atol=tolerance)]
nodetag2 = nodetag2[nodetag2['Domain'] == 'reg']
nodetag2 = nodetag2['tag'].values[0]
nodetag3 = nodes[np.isclose(nodes['x'], bstructure/2., atol=tolerance) & np.isclose(nodes['y'],  ly/2, atol=tolerance) & np.isclose(nodes['z'], 0, atol=tolerance)]
nodetag3 = nodetag3[nodetag3['Domain'] == 'structure']
nodetag3 = nodetag3['tag'].values[0]

file.write("if {$pid == %d}  {set recordList {%d %d %d}}\n" % (0, nodetag1, nodetag2, nodetag3))
file.close()