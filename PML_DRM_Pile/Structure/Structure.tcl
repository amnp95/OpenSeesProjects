...
if {$pid == 0} {
    ...
    catch {eval "exec $pythonexec Structure.py $lx $ly $lz $dx $dy $dz $pmlthicknessx $pmlthicknessy $pmlthicknessz $numpmllayers $drmthicknessx $drmthicknessy $drmthicknessz $numdrmlayers $regcores $drmcores $pmlcores $meshdir $outputdir"} result 
    puts "result: $result"
...
if {$pid < $regcores} {
    ...
    set nodesFile "results/nodeTags$pid.out"
    if {[file exists $nodesFile] == 1} { file delete $nodesFile }
    set nodesID [open $nodesFile "w"]
    puts $nodesID $recordList
    close $nodesID

    # write elements to a file
    set elementsFile "results/elementTags$pid.out"
    if {[file exists $elementsFile] == 1} { file delete $elementsFile }
    set elementsID [open $elementsFile "w"]
    puts $elementsID $elerecordList
    close $elementsID
}
barrier
# ============================================================================
