...
if {$pid == 0} {
    ...
    catch {eval "exec $pythonexec BeamEQStruct.py $lx $ly $lz $dx $dy $dz $pmlthicknessx $pmlthicknessy $pmlthicknessz $numpmllayers $drmthicknessx $drmthicknessy $drmthicknessz $numdrmlayers $regcores $drmcores $pmlcores $meshdir $outputdir"} result 
    puts "result: $result"
...
if {$pid <$regcores} {
    ...
    set beamNodes {}
    set beamElems {}
    set zlist    {3.0   2.5   2.0   1.5   1.0   0.75  0.5   0.25}
    set masslist {5400  10800 10800 10800 10800 5400  5400  0}
    set nNodePile [llength $zlist]
    # creating nodes
    for {set i 1} {$i <= $nNodePile} {incr i} {
        set zCoord [lindex $zlist [expr $i - 1]]
        set mass   [lindex $masslist [expr $i - 1]]
        node [expr $i + 1000000]   0.00   0.00   $zCoord -mass $mass 0 0 0 0 0
    ...
    set Iy            [expr $b*$b*$b*$h/12.]
    set Iz            [expr $b*$h*$h*$h/12.]
    set J             [expr $Iy + $Iz]
...
if {$pid < $regcores} {
    if {$DOPML=="YES" } {
        ...
        # eval "recorder Element -file results/PML/ElementStress$pid.out   -time -dT $deltaT -ele $elerecordList  stress"
        eval "recorder Element -file results/PML/Interfacepoints$pid.out -dT $deltaT -ele $interfaceElems  displacement"
        eval "recorder Node -file results/PML/BeamDisp$pid.out           -dT $deltaT -node $beamNodes  -dof 1 2 3 disp"
        eval "recorder Element -file results/PML/BeamForce$pid.out       -dT $deltaT -ele $beamElems  force"
    } else {
        ...
        # eval "recorder Element -file results/FIXED/ElementStress$pid.out   -dT $deltaT -ele $elerecordList  stress"
        eval "recorder Element -file results/FIXED/Interfacepoints$pid.out -dT $deltaT -ele $interfaceElems  displacement"
        eval "recorder Node -file results/FIXED/BeamDisp$pid.out           -dT $deltaT -node $beamNodes  -dof 1 2 3 disp"
        eval "recorder Element -file results/FIXED/BeamForce$pid.out       -dT $deltaT -ele $beamElems  force"
    }
